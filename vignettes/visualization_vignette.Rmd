...
g <- term_gene_graph(example_pathfindR_output, num_terms = NULL)
term_gene_plot(g)
```
...
```{r term_gene3}
g <- term_gene_graph(example_pathfindR_output, num_terms = 3, use_description = TRUE)
term_gene_plot(g)
```
...
term_gene_graph(example_pathfindR_output, num_terms = 3, term_size = "p_val")
```

`term_gene_graph` can also take the `genes_df` as input to visualise fold-changes, Fold Enrichments of the term nodes and an up-set like plot in graph context.
